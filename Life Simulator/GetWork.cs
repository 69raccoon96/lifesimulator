    ...
    public partial class GetWork : Form
    {
        ...
        private void TickTimer_Tick(object sender, EventArgs e)
        {
            ...
            foreach (int i in (from i in Enumerable.Range(0, field.GetLength(1))
                               where
(Enumerable.Range(0, field.GetLength(0)).Select(j => field[j, i]).Sum() >= width - 1)
        ...
        public new void Move(int x, int y)
        {
            ...
            if (FindMistake())
            {
                ...
                if (y != 0)
                {
                    ...
                    SetShape();
                    Form1.ALive += 1;
                    label1.Text = Convert.ToString(Form1.ALive);
                }
            }
            FillField();
        }
        ...
        {
            for (int i = 0; i < 4; i++)
                if (shape[1, i] >= width || shape[0, i] >= height || shape[1, i] <= 0 || shape[0, i] <= 0 ||
